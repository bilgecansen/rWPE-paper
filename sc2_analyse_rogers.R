...
dat_rogers <- read.csv("data_simulation/rogers_simulation_dataset_test.csv")

dat_st_25 <- filter(dat_rogers, Classification == "stochastic" & 
                      TimeSeriesLength == 25)
id_st_25 <- dat_st_25$ID %>% unique()

dat_st_50 <- filter(dat_rogers, Classification == "stochastic" & 
                      TimeSeriesLength == 50)
id_st_50 <- dat_st_50$ID %>% unique()

dat_pr_25 <- filter(dat_rogers, Classification == "periodic" & 
                      TimeSeriesLength == 25)
id_pr_25 <- dat_pr_25$ID %>% unique()

dat_pr_50 <- filter(dat_rogers, Classification == "periodic" & 
                      TimeSeriesLength == 50)
id_pr_50 <- dat_pr_50$ID %>% unique()

dat_ch_25 <- filter(dat_rogers, Classification == "chaotic" & 
                      TimeSeriesLength == 25)
id_ch_25 <- dat_ch_25$ID %>% unique()

dat_ch_50 <- filter(dat_rogers, Classification == "chaotic" & 
                      TimeSeriesLength == 50)
...
res_st_25 <- apply_wpe_test(dat_st_25, id_st_25)
res_st_50 <- apply_wpe_test(dat_st_50, id_st_50)
...
sig_st_25 <- map_dbl(res_st_25, function(x) which(x < 0.05) %>% length()/100)
sig_st_50 <- map_dbl(res_st_50, function(x) which(x < 0.05) %>% length()/100)
...
model_st <- unique(dat_st_25$Model)
model_pr <- unique(dat_pr_25$Model)
model_ch <- unique(dat_ch_25$Model)

df_st_25 <- data.frame(x = rep(as.factor(noise_level), times = length(model_st)),
                       y = rep(model_st, each = length(noise_level)),
                       z = sig_st_25)

df_st_50 <- data.frame(x = rep(as.factor(noise_level), times = length(model_st)),
                       y = rep(model_st, each = length(noise_level)),
                       z = sig_st_50)

df_st <- bind_rows(df_st_25, df_st_50) %>%
  add_column(ss = c(rep("25", 28), rep("50", 28)),
             model = c(rep("Stochastic", 28), rep("Stochastic", 28)))
...
res_rogers <- list(ch = df_ch,
                   pr = df_pr,
                   st= df_st)

saveRDS(res_rogers, "results/results_rogers.rds")
...
plot_pe_tiles <- function(df, title) {
  ...
  ggplot() +
    ...
    geom_text(data = df, mapping = aes(x = x, y = y, label = z), 
              color = "white", size = 4) +
    facet_wrap(~ model + ss) +
...
g1 <- plot_pe_tiles(df_st)
g2 <- plot_pe_tiles(df_pr)
g3 <- plot_pe_tiles(df_ch)


g1 / g2 / g3
ggsave("plot_rogers.jpeg", width = 9, height = 9, units = "in")
